...
import {
  ...
  resetConversationListLoadManagerForTest,
} from '../useConversationManagement';
...
describe('useConversationManagement', () => {
  ...
  it('should handle message streaming updates', async () => {
    ...
    mockSendMessageStream.mockImplementation(
      (_message: string, _convId: string, onChunk: (chunk: string, accumulated: string) => void) => {
  ...
  it('should complete summary flow after multi-turn message requires summary', async () => {
    const mockGeneratedSummary = 'Story summary generated by AI';
    const mockSaveSummary = vi.fn().mockResolvedValueOnce({
      conversation_id: 'conv_001',
    ...
    const mockGenerateSummary = vi
      .fn()
      .mockResolvedValueOnce(mockGeneratedSummary);
    ...
    await act(async () => {
      await result.current.handleSendMessage('Need a summary now');
    });

    expect(mockSummaryDialogOpen).toHaveBeenCalledWith('conv_001', 12);

    let generatedSummary = '';
    await act(async () => {
      generatedSummary = await result.current.handleGenerateSummary();
    });
    expect(generatedSummary).toBe(mockGeneratedSummary);

    await act(async () => {
      await result.current.handleSaveSummary(generatedSummary);
    });
    expect(mockSaveSummary).toHaveBeenCalledWith('conv_001', mockGeneratedSummary);
  });
  ...
  it('should dedupe concurrent manual loadConversations calls', async () => {
    let resolveList!: (value: ConversationWithSettings[]) => void;
    const listPromise = new Promise<ConversationWithSettings[]>((resolve) => {
      resolveList = resolve;
    });

    mockGetConversationsList.mockImplementation(() => listPromise);
    ...
    const callsBeforeManualLoad = mockGetConversationsList.mock.calls.length;

    let p1!: Promise<void>;
    let p2!: Promise<void>;
    let p3!: Promise<void>;
    await act(async () => {
      p1 = result.current.loadConversations();
      p2 = result.current.loadConversations();
      p3 = result.current.loadConversations();
    ...
    expect(mockGetConversationsList.mock.calls.length - callsBeforeManualLoad).toBe(
      1
    );
  });
});
