...
class TestCharacterService:
    ...
    def test_parse_story_unclosed_characters_tag(self, service):
        content = "Story line <CHARACTERS>unfinished block"
        story, _info, warnings = service.parse_story_with_characters(
            content, language="en"
        )
        assert "characters_tag_unclosed" in warnings
        assert "Story line" in story

    def test_parse_story_empty_characters_section(self, service):
        content = "Hi<CHARACTERS>  \n  </CHARACTERS>"
        story, _info, warnings = service.parse_story_with_characters(
            content, language="en"
        )
        assert "empty_characters_section" in warnings
        assert "Hi" in story

    def test_parse_story_valid_characters_block(self, service):
        content = (
            "Line1\n<CHARACTERS>\nCharacter Changes\n"
            "New Characters\n- Eve\n</CHARACTERS>"
        )
        story, info, warnings = service.parse_story_with_characters(
            content, language="en"
        )
        assert warnings == []
        assert "Eve" in info["new"]
        assert "Line1" in story
    ...
    def test_parse_characters_new_header_variants_en(
        self, service, header_line
    ):
        content = (
            f"X\n<CHARACTERS>\n{header_line}\n"
            "- Amy - Setting: brave knight\n</CHARACTERS>"
        )
        _story, info, warnings = service.parse_story_with_characters(
            content, language="en"
        )
        assert warnings == []
        assert "Amy" in info["new"]
    ...
    def test_parse_characters_status_header_variants_en(
        self, service, status_header
    ):
        content = (
            f"S\n<CHARACTERS>\n{status_header}\n"
            "- Bob - Became Main Character\n</CHARACTERS>"
        )
        _story, info, warnings = service.parse_story_with_characters(
            content, language="en"
        )
        assert warnings == []
        assert info["status_changes"].get("Bob", {}).get("is_main") is True
